...
class UserService {
  ...
  async create(payload: RegisterInput): Promise<UserResponse> {
    ...
    if (userResponse) {
      throw new CustomError(ERROR_TYPE.CONFLICT, 'email', `User with email: ${email} already exists`);
    }
  ...
  async verify(payload: LoginInput): Promise<UserResponse> {
    ...
    const userResponse = await this.getByEmail(email);
    if (!userResponse) {
      throw new CustomError(ERROR_TYPE.CONFLICT, 'email', `User with email: ${email} doesn't exist`);
    }

    const validUser = await this.comparePasswords(password, userResponse.password);
    if (!validUser) {
      throw new CustomError(ERROR_TYPE.CONFLICT, 'password', 'Invalid password');
    }
    return userResponse;
  }
}
